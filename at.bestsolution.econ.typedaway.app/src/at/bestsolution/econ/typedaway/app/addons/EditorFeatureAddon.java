package at.bestsolution.econ.typedaway.app.addons;
...
public class EditorFeatureAddon {
	...
	public void init() {
		itemMap = new HashMap<>();
		MMenu menu = (MMenu) modelService.findElements(application, "at.bestsolution.econ.typedaway.app.menu.editor", null, null, EModelService.ANYWHERE | EModelService.IN_MAIN_MENU).get(0);
	...
	private MMenuItem createFeature(Feature f) {
		...
		e.setContributorURI("platform:/plugin/at.bestsolution.econ.typedaway.app");
		...
		application.getCommands()
			.stream()
			.filter( c -> c.getElementId().equals("at.bestsolution.econ.typedaway.app.command.toggleeditorfeature"))
			.findFirst()
